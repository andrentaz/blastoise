...
typedef Digraph::ArcMap<int> ArcIndexMap;
...
typedef Digraph::NodeMap<int> DNodeIndexMap;
...
public:
	AdjacencyMatrixDirected(ListDigraph &dgraph,ArcValueMap &graphweight,double NonEdgeValue);
	~AdjacencyMatrixDirected();
	double *AdjMatrix;
	ListDigraph *g;
	ArcValueMap *weight;
	int Nnodes,Narcs,Nmatrix;
	double NonArcValue;
	DNode *Index2DNode;
	Arc *Index2Arc;
	double Cost(DNode,DNode);
	double Cost(Arc);
	DNodeIndexMap DNode2Index;
	ArcIndexMap Arc2Index;
};
