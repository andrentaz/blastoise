           ...
           NodeBoolMap& is_terminal)
{
  ...
  if (m==-1) r = ReadEuclideanListGraph(filename,g,nodename,custo,posx,posy,is_terminal);
...
double AdjacencyMatrixDirected::Cost(DNode u,DNode v)
{
    int i_u,i_v;
    i_u = DNode2Index[u];
    i_v = DNode2Index[v];
    ...
    //cout << "globalcounter1 = " << globalcounter << endl;
    try{
        if( i_u < Nnodes && i_v < Nnodes ) return AdjMatrix[i_u*Nnodes+i_v];
        else return(NonArcValue);
    }catch (...) {
        cout << "Exception: " << globalcounter << endl;
        exit(1);
    }
...
double AdjacencyMatrixDirected::Cost(Arc e)
{
    int i_u,i_v;
    DNode u,v;
    globalcounter++;
    ...
    i_u = DNode2Index[u];
    i_v = DNode2Index[v];
    return AdjMatrix[i_u*Nnodes+i_v];
}


AdjacencyMatrixDirected::~AdjacencyMatrixDirected()
{
    ADJMAT_FreeNotNull(AdjMatrix); ADJMAT_FreeNotNull(Index2DNode); ADJMAT_FreeNotNull(Index2Arc);
}
